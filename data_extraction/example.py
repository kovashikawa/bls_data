...
def run_basic_example():
    ...
    try:
        ...
        if not bls_dataframe.empty:
            log.info("\n" + "="*80)
            log.info("Successfully fetched BLS data (API-only mode). Displaying a sample:")
            log.info("="*80 + "\n")
            ...
            log.warning("The request was successful, but no data was returned for the specified series and years.")
        ...
        log.error("Please ensure all requested aliases exist in your 'code_mapping.csv' file.")
...
def run_database_example():
    ...
    try:
        ...
        log.info(f"   Same number of rows: {len(bls_dataframe_api) == len(bls_dataframe_cache)}")
        ...
        if not bls_dataframe_cache.empty:
            ...
            log.info(f"Date range: {bls_dataframe_cache['year'].min()}-{bls_dataframe_cache['year'].max()}")
...
def run_example():
    ...
    if database_available:
        ...
        log.info("   To enable database features, ensure PostgreSQL is running and configured")
    ...
    if database_available:
        ...
        log.info("\nNext steps:")
        log.info("• Try the command line interface: python -m bls_data.data_extraction.main cpi_all_items --use-database")
