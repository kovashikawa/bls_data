...
def test_date_column_queries():
    ...
    with db_config.get_session() as session:
        ...
        total_with_dates = session.query(BLSDataPoint).filter(
        ...
        # 2023 data
        data_2023 = session.query(BLSDataPoint).filter(
        ...
        # 2024 data
        data_2024 = session.query(BLSDataPoint).filter(
            BLSDataPoint.date >= date(2024, 1, 1),
        ...
        # Last 5 years
        data_last_5_years = session.query(BLSDataPoint).filter(
        ...
        yearly_counts = session.query(
            extract('year', BLSDataPoint.date).label('year'),
            func.count(BLSDataPoint.id).label('count')
        ).filter(
            BLSDataPoint.date.isnot(None)
        ).group_by(extract('year', BLSDataPoint.date)).order_by('year').all()
        ...
        recent_data = session.query(BLSDataPoint).filter(
            BLSDataPoint.date >= date(2023, 1, 1)
        ).count()
        log.info(f"✅ Recent data (2023+): {recent_data:,} points")
        
        # Specific month
        march_2023 = session.query(BLSDataPoint).filter(
        ...
        # Quarterly data
        q1_2023 = session.query(BLSDataPoint).filter(
            BLSDataPoint.date >= date(2023, 1, 1),
            BLSDataPoint.date < date(2023, 4, 1)
        ).count()
        ...
        cpi_data = session.query(BLSDataPoint).filter(
            BLSDataPoint.series_id == 'CUUR0000SA0',
            BLSDataPoint.date >= date(2020, 1, 1)
        ).order_by(BLSDataPoint.date).limit(10).all()
