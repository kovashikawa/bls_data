...
class BLSFormatter(logging.Formatter):
    ...
    def format(self, record: logging.LogRecord) -> str:
        ...
        if record.levelno >= logging.ERROR and record.exc_info and self.detailed_errors:
    ...
    def _get_caller_function(self) -> str:
        ...
        try:
            ...
            for _ in range(5):  # Limit search depth
                ...
                if (
                    frame.f_code.co_filename.endswith("logging/")
...
class PerformanceFormatter(logging.Formatter):
    ...
    def __init__(self):
        ...
        super().__init__(
            "%(asctime)s [PERF] %(funcName)s: %(message)s", datefmt="%Y-%m-%d %H:%M:%S"
        )
