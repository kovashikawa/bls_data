...
from database.utils import setup_database, load_initial_series_metadata, get_database_stats, create_database_indexes
...
def basic_database_operations_example():
    ...
    try:
        ...
        with db_config.get_session() as session:
            ...
            log.info(f"Found {len(search_results)} series matching 'consumer price index'")
            ...
            if search_results:
                ...
                for series_id, info in freshness.items():
                    log.info(f"  - {series_id}: Last extracted: {info.get('last_extracted', 'Never')}")
    ...
    try:
        ...
        with db_config.get_session() as session:
            ...
            if freshness_data:
                ...
                fresh_count = sum(1 for info in freshness_data.values() 
                                if info.get('last_extracted') is not None)
                log.info(f"   Series with extraction history: {fresh_count}")
                log.info(f"   Series never extracted: {len(freshness_data) - fresh_count}")
            else:
    ...
    try:
        ...
        with db_config.get_session() as session:
            ...
            if not df_db.empty:
                ...
                log.info("   No data found in database (try running data extraction first)")
...
def main():
    ...
    try:
        ...
        if not db_config.check_connection():
            log.info("❌ Database connection failed. Please check your database configuration.")
            log.info("   Make sure PostgreSQL is running and your .env file is configured correctly.")
            return
    except Exception as e:
    ...
    try:
        ...
        log.info("   python -m bls_data.data_extraction.main cpi_all_items --use-database")
