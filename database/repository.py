...
class BLSDataRepository:
    ...
    def upsert_series_data(self, series_data: List[Dict], 
                          extraction_id: str = None) -> Tuple[int, int, int]:
        ...
        try:
            for series_item in series_data:
                ...
                series = self.session.query(BLSSeries).filter_by(series_id=series_id).first()
                if not series:
                    series = BLSSeries(
                        series_id=series_id,
                    ...
                    series.series_title = series_item.get('series_title', series.series_title)
                    series.survey_name = series_item.get('survey_name', series.survey_name)
                    series.measure_data_type = series_item.get('measure_data_type', series.measure_data_type)
                    series.area = series_item.get('area', series.area)
                    series.item = series_item.get('item', series.item)
                    series.seasonality = series_item.get('seasonality', series.seasonality)
                ...
                for data_point in series_item.get('data', []):
                    ...
                    existing_point = self.session.query(BLSDataPoint).filter_by(
            ...
            log.info(f"Data upsert completed: {inserted_count} inserted, {updated_count} updated")
    ...
    def get_series_data(self, series_ids: List[str], 
                       start_year: int = None, 
                       end_year: int = None,
                       include_metadata: bool = True) -> pd.DataFrame:
        """Retrieve data from database with optional filtering"""
        try:
            query = self.session.query(BLSDataPoint, BLSSeries).join(
                BLSSeries, BLSDataPoint.series_id == BLSSeries.series_id
            ...
            results = query.order_by(BLSDataPoint.series_id, BLSDataPoint.year, BLSDataPoint.period).all()
    ...
    def get_data_freshness(self, series_ids: List[str]) -> Dict[str, Dict]:
        ...
        try:
            freshness_data = self.session.query(BLSDataFreshness).filter(
                BLSDataFreshness.series_id.in_(series_ids)
            ).all()
            ...
            for freshness in freshness_data:
                result[freshness.series_id] = {
                    ...
                    'data_completeness': float(freshness.data_completeness) if freshness.data_completeness else 0.0,
    ...
    def mark_series_updated(self, series_id: str, extraction_id: str):
        """Update freshness tracking"""
        try:
            freshness = self.session.query(BLSDataFreshness).filter_by(series_id=series_id).first()
            if not freshness:
    ...
    def get_stale_series(self, max_age_hours: int = 24) -> List[str]:
        ...
        try:
            ...
            stale_series = self.session.query(BLSDataFreshness.series_id).filter(
                or_(
    ...
    def search_series(self, query: str, limit: int = 100) -> List[Dict]:
        ...
        try:
            ...
            search_query = self.session.query(BLSSeries).filter(
                func.to_tsvector('english', 
                    func.concat(BLSSeries.series_title, ' ', 
    ...
    def log_extraction(self, extraction_id: str, series_ids: List[str], 
                      start_year: int = None, end_year: int = None,
                      status: str = 'success', records_extracted: int = 0,
                      records_updated: int = 0, records_inserted: int = 0,
                      error_message: str = None, api_calls_made: int = 0,
