...
def load_initial_series_metadata(csv_path: str = None) -> int:
    ...
    if not csv_path:
        ...
        csv_path = Path(__file__).parent.parent / "cu_series" / "cpi_series_master_list.csv"
...
def get_database_stats() -> Dict:
    ...
    try:
        from .config import DatabaseConfig
        from .models import BLSSeries, BLSDataPoint, BLSExtractionLog
        ...
        with db_config.get_session() as session:
            stats = {
                ...
                'latest_extraction': session.query(BLSExtractionLog).order_by(BLSExtractionLog.created_at.desc()).first(),
            }
...
def cleanup_old_data(retention_days: int = 365):
    ...
    try:
        ...
        with db_config.get_session() as session:
            deleted_count = session.query(BLSExtractionLog).filter(
                BLSExtractionLog.created_at < cutoff_date
            ).delete()
...
def backup_database(backup_path: str = None):
    ...
    try:
        ...
        if not backup_path:
            backup_path = f"bls_data_backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}.sql"
