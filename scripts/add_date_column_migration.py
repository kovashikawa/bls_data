...
def add_date_column():
    ...
    try:
        with db_config.get_session() as session:
            ...
            data_points = (
                session.query(BLSDataPoint).filter(BLSDataPoint.date.is_(None)).all()
            )
                ...
                for i, point in enumerate(data_points):
                    try:
                        ...
                        if (i + 1) % 1000 == 0:
                            session.commit()
                            log.info(
                                f"   Processed {i + 1}/{len(data_points)} data points..."
                            )
            ...
            for point in sample_points:
                log.info(
                    f"   {point.series_id} | {point.year} {point.period} → {point.date.strftime('%Y-%m-%d')}"
                )
...
def main():
    ...
    if success:
        ...
        log.info(
            "3. Example query: SELECT * FROM bls_data_points WHERE date >= '2020-01-01'"
        )
        return 0
    else:
